...
## ⚠️ 4択（grammar）の場合は別ワークフロー

以下のいずれかに該当する場合は **このコマンドではなく `/g-prepare` → `/g-render`** を使うこと:

- フォルダ末尾が `/grammar`
- フォルダ内に `grammar.md` / `ichimondittou.md` / `ichimondittou_01.md` が存在

英文法トピックは4択穴埋め形式（FourChoice）で動画化する。一問一答（IchimonItto）として `XX-qas.md` を作るのは誤り。
