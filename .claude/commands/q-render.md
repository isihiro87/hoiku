...
**⚠️ 注意**: `bc`コマンドが使えない環境があるため、**Node.jsを使用する**:

```bash
# Node.jsで音声長さを計算（推奨）
cd [フォルダパス]
node -e "const fs=require('fs'); const files=fs.readdirSync('audios').filter(f=>f.endsWith('.wav')).sort(); files.forEach(f=>{const size=fs.statSync('audios/'+f).size; const frames=Math.round((size-44)*1000/96000*30/1000); console.log(f+': '+frames);});"
```
...
cp /workspaces/hoiku_shiken/[フォルダパス]/audios/[開始番号]-[トピックID].wav "$TEMP_DIR/audio/explain/"
cp /workspaces/hoiku_shiken/[フォルダパス]/audios/[開始番号+1]-[トピックID].wav "$TEMP_DIR/audio/explain/"
# ... 必要な音声ファイルをすべて列挙

cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"

# データファイルをsrcにコピー
cp /workspaces/hoiku_shiken/[フォルダパス]/[ID]-IchimonIttoData.tsx src/IchimonIttoData.tsx

echo "=== [ID] レンダリング開始 ==="
PUPPETEER_HEADLESS_MODE=new npx remotion render src/index.ts IchimonIttoShorts \
  /workspaces/hoiku_shiken/[フォルダパス]/[ID]-output.mp4 \
  --public-dir "$TEMP_DIR" \
...
**⚠️ 高速化のポイント**:
- **一時ディレクトリ使用**: `--public-dir "$TEMP_DIR"` で必要なファイルのみ参照（全public/を読まない）
- **`--concurrency 4`**: 並列処理（CPUコア数に応じて調整可）
- **`--gl swiftshader`**: ソフトウェアレンダリング（環境依存を減らす）

保存先: `[フォルダパス]/[ID]-render.sh`（quizフォルダ内に保存）

**例**: `datas/history/grade3/6-4/4end_of_war/quiz/qas-render.sh`
...
bash [フォルダパス]/[ID]-render.sh

複数ファイルがある場合は順番に実行してください:
1. bash [フォルダパス]/qas-render.sh
2. bash [フォルダパス]/qas2-render.sh
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**実行例**:
```bash
bash datas/history/grade3/6-4/4end_of_war/quiz/qas-render.sh
bash datas/history/grade3/6-4/4end_of_war/quiz/qas2-render.sh
```
