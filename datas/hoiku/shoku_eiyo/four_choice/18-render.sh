...
cp "$BASE/audios/18-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/18-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/18-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/18-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/18-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/18-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/18-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/18-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/18-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 18 家庭や児童福祉施設における食事と栄養② (4択) レンダリング開始 ==="
