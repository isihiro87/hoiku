...
cp "$BASE/audios/21-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/21-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/21-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/21-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/21-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/21-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/21-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/21-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/21-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 21 特別な配慮を要する子どもの食と栄養① (4択) レンダリング開始 ==="
