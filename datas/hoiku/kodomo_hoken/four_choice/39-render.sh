...
cp "$BASE/audios/39-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/39-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/39-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/39-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/39-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/39-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/39-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/39-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/39-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 39 子どもの疾病の予防及び適切な対応③ (4択) レンダリング開始 ==="
