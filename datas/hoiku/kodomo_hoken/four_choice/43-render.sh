...
cp "$BASE/audios/43-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/43-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/43-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/43-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/43-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/43-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/43-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/43-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/43-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 43 子どもの健康と安全② (4択) レンダリング開始 ==="
