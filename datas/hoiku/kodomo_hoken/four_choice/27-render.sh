...
cp "$BASE/audios/27-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/27-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/27-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/27-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/27-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/27-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/27-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/27-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/27-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 27 子どもの心身の健康状態とその把握④ (4択) レンダリング開始 ==="
