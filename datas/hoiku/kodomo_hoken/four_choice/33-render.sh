...
cp "$BASE/audios/33-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/33-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/33-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/33-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/33-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/33-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/33-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/33-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/33-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 33 子どもの心身の健康状態とその把握⑩ (4択) レンダリング開始 ==="
