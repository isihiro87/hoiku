...
cp "$BASE/audios/35-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/35-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/35-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/35-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/35-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/35-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/35-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 35 子どもの心身の健康状態とその把握 (4択) レンダリング開始 ==="
