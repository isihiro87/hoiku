...
cp "$BASE/audios/42-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/42-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/42-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/42-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/42-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/42-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/42-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/42-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/42-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 42 子どもの健康と安全① (4択) レンダリング開始 ==="
