...
cp "$BASE/audios/26-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/26-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/26-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/26-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/26-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/26-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/26-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/26-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/26-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 26 子どもの心身の健康状態とその把握③ (4択) レンダリング開始 ==="
