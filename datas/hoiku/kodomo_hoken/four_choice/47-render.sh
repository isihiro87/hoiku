...
cp "$BASE/audios/47-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/47-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/47-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/47-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/47-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/47-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/47-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 47 子どもの健康と安全⑥ (4択) レンダリング開始 ==="
