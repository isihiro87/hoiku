...
cp "$BASE/audios/20-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/20-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/20-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/20-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/20-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/20-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/20-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/20-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/20-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 20 子どもの身体的発育・発達と保健 (4択) レンダリング開始 ==="
