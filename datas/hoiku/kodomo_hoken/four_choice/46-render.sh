...
cp "$BASE/audios/46-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/46-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/46-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/46-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/46-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/46-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/46-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 46 子どもの健康と安全⑤ (4択) レンダリング開始 ==="
