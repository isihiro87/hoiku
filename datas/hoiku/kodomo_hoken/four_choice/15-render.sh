...
cp "$BASE/audios/15-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/15-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/15-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/15-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/15-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/15-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/15-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/15-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/15-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 15 子どもの身体的発育・発達と保健⑩ (4択) レンダリング開始 ==="
