...
cp "$BASE/audios/49-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/49-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/49-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/49-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/49-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/49-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/49-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/49-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/49-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 49 保育における保健活動の計画及び評価① (4択) レンダリング開始 ==="
