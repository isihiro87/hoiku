...
cp "$BASE/audios/10-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/10-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/10-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/10-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/10-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/10-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/10-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/10-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/10-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 10 子どもの身体的発育・発達と保健⑤ (4択) レンダリング開始 ==="
