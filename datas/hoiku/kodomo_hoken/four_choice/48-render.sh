...
cp "$BASE/audios/48-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/48-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/48-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/48-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/48-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/48-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/48-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 48 子どもの健康と安全⑦ (4択) レンダリング開始 ==="
