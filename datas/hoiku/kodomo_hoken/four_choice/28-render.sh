...
cp "$BASE/audios/28-00-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/28-01-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/28-02-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/28-03-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/28-04-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/28-05-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/28-06-voice.wav" "$TEMP_DIR/audio/explain/"
cp "$BASE/audios/28-07-voice.wav" "$TEMP_DIR/audio/explain/"
cp public/audio/bgm.mp3 "$TEMP_DIR/audio/"
[ -f public/audio/finish.wav ] && cp public/audio/finish.wav "$TEMP_DIR/audio/explain/" || true
cp "$BASE/28-FourChoiceData.tsx" src/FourChoiceData.tsx

echo "=== 28 子どもの心身の健康状態とその把握⑤ (4択) レンダリング開始 ==="
